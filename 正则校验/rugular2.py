...
import re
import numpy as np
from regular_data2 import all_reg_dic
...
# 鉴权问题-验证次数超限
dataset1 =Dataset_load("test2.xlsx","Sheet1")
# 鉴权问题-银行原因异常
dataset2 =Dataset_load("test2.xlsx","Sheet2")
# 交易受限-不能跨省交易
dataset3 =Dataset_load("test2.xlsx","Sheet3")
# 交易受限-非受理时间
dataset4 =Dataset_load("test2.xlsx","Sheet4")
# 交易限额超限
dataset5 =Dataset_load("test2.xlsx","Sheet5")
# 交易异常
dataset6=Dataset_load("test2.xlsx","Sheet6")
# 交易异常-币别或钞汇鉴别不存在
dataset7=Dataset_load("test2.xlsx","Sheet7")
# 交易异常-不允许交易
dataset8=Dataset_load("test2.xlsx","Sheet8")
# 交易异常-付款合法性检查失败
dataset9=Dataset_load("test2.xlsx","Sheet9")
#交易异常-缴费协议有误
dataset10=Dataset_load("test2.xlsx","Sheet10")
# 交易异常-通兑问题
dataset11=Dataset_load("test2.xlsx","Sheet11")
# 交易异常—未对应凭证
dataset12=Dataset_load("test2.xlsx","Sheet12")
# 交易异常-账户休眠
dataset13=Dataset_load("test2.xlsx","Sheet13")
# 交易异常-支付渠道问题
dataset14=Dataset_load("test2.xlsx","Sheet14")
# 没有符合条件的记录
dataset15=Dataset_load("test2.xlsx","Sheet15")
# 未找到业务流水号
dataset16=Dataset_load("test2.xlsx","Sheet16")
# 系统问题
dataset17=Dataset_load("test2.xlsx","Sheet17")
# 系统异常
dataset18=Dataset_load("test2.xlsx","Sheet18")
# 银行端通讯异常
dataset19=Dataset_load("test2.xlsx","Sheet19")
# 余额不足
dataset20=Dataset_load("test2.xlsx","Sheet20")
# 账户类型错误
dataset21=Dataset_load("test2.xlsx","Sheet21")
# 账户密码异常
dataset22=Dataset_load("test2.xlsx","Sheet22")
# 账户信息异常
dataset23=Dataset_load("test2.xlsx","Sheet23")
# 账户状态异常
dataset24=Dataset_load("test2.xlsx","Sheet24")

data_dict = {'鉴权问题-验证次数超限':dataset1,'鉴权问题-银行原因异常':dataset2,'交易受限-不能跨省交易':dataset3,'交易受限-非受理时间':dataset4,
             '交易限额超限':dataset5,'交易异常':dataset6,'交易异常-币别或钞汇鉴别不存在':dataset7,'交易异常-不允许交易':dataset8,
             '交易异常-付款合法性检查失败':dataset9,'交易异常-缴费协议有误':dataset10,'交易异常-通兑问题':dataset11,'交易异常—未对应凭证':dataset12,
             '交易异常-账户休眠':dataset13,'交易异常-支付渠道问题':dataset14,'没有符合条件的记录':dataset15,'未找到业务流水号':dataset16,
             '系统问题':dataset17,'系统异常':dataset18,'银行端通讯异常':dataset19,'余额不足':dataset20,'账户类型错误':dataset21,
             '账户密码异常':dataset22,'账户信息异常':dataset23,'账户状态异常':dataset24}


def re_match_one(data_dict,all_reg_dic):
    list1 = []
    list2 = []

    for data_tag,data_content_list in data_dict.items():
        for data_content in data_content_list:
            num1 = 0
            list3 = []
            for reg_tag,reg_list in all_reg_dic.items():

                for reg in reg_list:
                    result = re.search(reg, data_content)
                    if result:
                        list3.append({reg_tag:reg})
                        if data_tag == reg_tag:
                            num1 += 1
                        else:
                            list2.append({data_content:[data_tag,'-错误匹配-',reg_tag,reg]})



            if num1 == 0:
                list1.append({data_tag:data_content})

    for a in list2:
        print(a)
    print(len(list2))
    print("===========================================================================")
    print('无法匹配',list1,len(list1))
...
re111 = re_match_one(data_dict,all_reg_dic)
